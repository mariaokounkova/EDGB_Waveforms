...
def ReadExtrapolatedMode(p, piece, mode):
  """ Given a file of extrapolated modes, read in the (mode)
      at a given order """
  piece_dict = {"hRWZ" : "/rhOverM_Extrapolated_N2.h5", \
          "DeltaPsi4" : "/rMDeltaPsi4_Extrapolated_N2.h5"}
  try:
...
def ComputeStrain(time, psi4, data_dir):
    ...
    mass = 1.0 ## TODO TEMP FOR NOW ChristodolouMass(data_dir) 
...
def ComputeEDGBDeltaStrain(p, mode):
  ...
  ## Pad the delta strain array to be the length of the simulation
...
def OutputEDGBDeltaStrain(p, only22):
  """ Given a path p to the extrapolated hPsi4, 
      compute the modification to the gravitational wave strain. 
      Return time time, strain, and delta_strain """
  ...
  for l in l_arr:
    ...
    for m in range(-l, l+1) if not only22 else [2, -2]:
      ...
      time, strain, delta_strain = ComputeEDGBDeltaStrain(p, mode)
...
def main():
  ...
  OutputEDGBDeltaStrain(data_dir, args.only22)
